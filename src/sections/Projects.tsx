    ...
    >
      ...
      <div className="flex flex-wrap justify-center gap-12">
        {projects.items.map((project, index) => (
          <div key={project.id} className="w-full md:w-[calc(50%-24px)] lg:w-[calc(33.333%-32px)]">
            <Surface
              variant="elevated"
              ...
              tabIndex={0} role="button" aria-label={`open ${project.title}`}
              className={cn(
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.technologies.slice(0, 3).map((tech) => (
                  <span key={tech} className="px-2 py-0.5 bg-secondary text-secondary-foreground font-hand text-xs rounded-sm border border-transparent hover:border-ink/20">
                    {tech}
                  </span>
                ))}
